...
def write_const_T_map(T0, fname, nside=16):
    ...
    stokes[0, 0] = T0 * u.K
    frequency = 10 * u.MHz # naturally in MHz
    skymodel = pyradiosky.SkyModel(
        ...
        spectral_index= 0*np.ones(npix),
    )
    assert skymodel.check()
    skymodel.write_skyh5(fname,clobber=True)


if __name__ == "__main__":
    print("Making T=1*10^4k monopole only map...")
    write_const_T_map(1e4, "data/const_1e4K_32.hdf5", nside=32)
